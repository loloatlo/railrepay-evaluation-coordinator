 ...
 *
 * AC-6: Parse KAFKA_BROKERS, KAFKA_USERNAME, KAFKA_PASSWORD, KAFKA_GROUP_ID, KAFKA_SSL
 * AC-7: Graceful degradation - throw ConsumerConfigError when config missing
...
export function createConsumerConfig(): ConsumerConfig {
  ...
  // Get service name (default to evaluation-coordinator)
  const serviceName = process.env.SERVICE_NAME || 'evaluation-coordinator';
